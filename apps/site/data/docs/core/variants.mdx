...
The special keys `true` and `false` will map to a boolean. So the `selectable` prop will be typed to accept true or false, and when true it will apply it's styles.
...
#### Extra properties passed to variants
